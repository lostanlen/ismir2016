...
def predict(graph, is_spiral, X_test, offset):
    if is_spiral:
        Q = 12
        X0 = X_test * window(X_test, 0, 4*Q, 3*Q) - offset / 2
        X1 = X_test * window(X_test, 3*Q+1, 5*Q, 4*Q) - offset / 2
...
def train_on_batch(graph, is_spiral, X_batch, Y_batch, offset):
    if is_spiral:
        Q = 12
        X0 = X_batch * window(X_batch, 0, 5*Q, 1*Q) - offset / 2
        X1 = X_batch * window(X_batch, 3*Q, 5*Q, 4*Q) - offset / 2
...
def window(X_batch, start, full_width, top_width=None):
    ...
    alpha = float(top_width) / full_width
